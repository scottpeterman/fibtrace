...
class JunosParser:
    ...
    def parse_fib(raw: str, target_prefix: str) -> Optional[FibEntry]:
        ...
        try:
            ...
            # Junos ECMP routes wrap multiple <nh> inside a parent
            # <nh><nh-type>ulst</nh-type>...<nh>...</nh><nh>...</nh></nh>
            # so we must unpack the wrapper to reach the real next-hops.
            fib_nhs = []
            state = FibState.PROGRAMMED

            def _process_nh(nh_elem):
                """Process a single <nh> element, return True if state was set."""
                nonlocal state
                nh_type = _xml_text(nh_elem, 'nh-type').lower()

                if 'discard' in nh_type or 'reject' in nh_type:
                    state = FibState.DROP
                    return
                if 'receive' in nh_type or 'local' in nh_type:
                ...
                # ECMP list wrapper — recurse into child <nh> elements
                if 'ulst' in nh_type or 'unilist' in nh_type:
                    for child_nh in _xml_find_all(nh_elem, 'nh'):
                        _process_nh(child_nh)
                    return

                to_addr = _xml_text(nh_elem, 'to')
                via_intf = _xml_text(nh_elem, 'via')
...
class CiscoIOSParser:
    ...
    # ── IOS Route Next-Hop Patterns ──────────────────────────
    # Ranked list: specific → general.  First pattern to produce
    # matches wins.  Each tuple:
    #   (name, regex, ip_group, intf_group_or_None)
    #
    # All patterns are applied against the text AFTER
    # "Routing Descriptor Blocks:".  If that section is absent,
    # patterns are applied to the full output.
    #
    # To add a new IOS variant: insert a new tuple at the
    # appropriate specificity rank and add a regression test.
    # ──────────────────────────────────────────────────────────

    _ROUTE_NH_PATTERNS: list[tuple[str, str, int, int | None]] = [
        # 1. Full format (OSPF, BGP, EIGRP with from/via)
        #    * 172.16.1.1, from 172.16.1.1, 00:05:32 ago, via GigabitEthernet0/1
        (
            "full_from_via",
            r'(?:\*\s+)?([\d\.]+),\s+from\s+[\d\.]+,.*?via\s+(\S+)',
            1, 2,
        ),
        # 2. Static/default with via interface
        #    * 10.1.1.1, via GigabitEthernet0/0
        #        Route metric is 0, traffic share count is 1
        (
            "static_via",
            r'\*?\s*([\d]+\.[\d]+\.[\d]+\.[\d]+)\s*,\s*via\s+(\S+)'
            r'\s*\n\s+Route metric is',
            1, 2,
        ),
        # 3. Static/default bare next-hop (no from, no via)
        #    * 172.16.10.1
        #        Route metric is 0, traffic share count is 1
        (
            "static_bare",
    ...
    def parse_route(raw: str, target_prefix: str) -> Optional[RouteEntry]:
        ...
        if re.search(r"not in table|% Network not in table|% Subnet not in table",
                      raw, re.IGNORECASE):
            return None

        try:
            # Extract prefix (strip trailing comma from e.g. "0.0.0.0/0, supernet")
            m = re.search(r"Routing entry for (\S+?)(?:,|\s|$)", raw)
            if not m:
                return None
            ...
            # ── Next-hop extraction via ranked pattern list ──
            # Isolate descriptor block if present
            parts = re.split(r'Routing Descriptor Blocks:', raw, maxsplit=1)
            nh_text = parts[1] if len(parts) == 2 else raw

            next_hops = []
            matched_pattern = None

            for name, pattern, ip_grp, intf_grp in CiscoIOSParser._ROUTE_NH_PATTERNS:
                hits = list(re.finditer(pattern, nh_text))
                if hits:
                    for m in hits:
                        nh_addr = _safe_ip(m.group(ip_grp))
                        nh_intf = (m.group(intf_grp).rstrip(',')
                                   if intf_grp and m.group(intf_grp)
                                   else None)
                        next_hops.append(RouteNextHop(
                            address=nh_addr,
                            interface=nh_intf,
            ...
            if matched_pattern:
                logger.debug(
                    f"IOS route NH matched pattern '{matched_pattern}': "
                    f"{len(next_hops)} next-hop(s)"
                )
    ...
    def parse_fib(raw: str, target_prefix: str) -> Optional[FibEntry]:
        ...
        try:
            ...
            # Check for recursive via (must come BEFORE bare "attached" check)
            # Format:
            #   recursive via 172.16.10.1
            #     attached to Vlan10
            # May have multiple recursive entries (ECMP)
            recursive_nhs = []
            for m in re.finditer(
                r'recursive via\s+([0-9a-fA-F:\.]+)\s*\n'
                r'\s+attached to\s+(\S+)',
                raw
            ):
                recursive_nhs.append(FibNextHop(
            ...
            if recursive_nhs:
                return FibEntry(
                    prefix=prefix,
                    state=FibState.PROGRAMMED,
                    next_hops=recursive_nhs,
                    resolved=True,
                )

            # Check for "attached" (directly connected, no next-hop IP)
