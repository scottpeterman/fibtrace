...
IOS_ROUTE_PATTERNS = {
    # "Routing entry for 10.0.0.0/24" or "Routing entry for 0.0.0.0/0, supernet"
    "prefix": r"Routing entry for (\S+?)(?:,|\s|$)",
    # "Known via "ospf 1", distance 110, metric 20"
    "protocol": r'Known via "(\S+)',
    # Next-hop patterns — ranked list in CiscoIOSParser._ROUTE_NH_PATTERNS
    # 1. full_from_via:  "* 172.16.1.1, from 172.16.1.1, via GigabitEthernet0/1"
    # 2. static_via:     "* 10.1.1.1, via GigabitEthernet0/0"
    # 3. static_bare:    "* 172.16.10.1"  (followed by "Route metric is")
    "next_hop": r"(see CiscoIOSParser._ROUTE_NH_PATTERNS — ranked pattern list)",
}
...
IOS_CEF_PATTERNS = {
    ...
    "recursive": r"recursive via\s+([0-9a-fA-F:\.]+)\s*\n\s+attached to\s+(\S+)",
    # "attached to GigabitEthernet0/1"  (bare, no recursive — glean)
