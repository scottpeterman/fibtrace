...
from .forwarding_graph import ForwardingGraph
...
# IMPORTANT: Match with surrounding spaces to avoid substring collisions
# (e.g., "ae10" matching "ae100", "ae101", "Po10" matching "Po100").
...
class WalkerConfig:
    ...
    # Next-hop source preference:
    #   "fib"  — use forwarding-table next-hops, fall back to RIB (default)
    #   "rib"  — use RIB next-hops, fall back to FIB
    #   "auto" — use whichever has more next-hops (catches ECMP missed by FIB parser)
    nh_source_preference: str = "fib"

    # DNS domain suffix for neighbor hostname resolution
    # e.g., "company.com" → "edge03.iad1" becomes "edge03.iad1.company.com"
...
class ChainWalker:
    ...
    def __init__(self, config: WalkerConfig):
        ...
        self._graph: Optional[ForwardingGraph] = None
    ...
    @property
    def graph(self) -> Optional[ForwardingGraph]:
        return self._graph
    ...
    def walk(self) -> ForwardingChain:
        ...
        self._graph = ForwardingGraph(
            target_prefix=self.config.target_prefix,
            source_host=self.config.source_host,
        )
        self._graph.started_at = started_at
        ...
        while queue:
            ...
            if (client is None
                ...
                candidates = self._resolve_via_neighbors(item)
                for candidate_ip in candidates:
                    client = self._connect(candidate_ip)
                    if client:
                        resolved_via = candidate_ip
                        logger.info(
                            f"Reached {item.ssh_target} via neighbor "
                            f"discovery → {candidate_ip}"
                        )
                        item.ssh_target = candidate_ip
                        break
                    else:
                        self._progress(
                            f"  ↪ {candidate_ip} SSH failed, "
                            f"trying next candidate..."
                        )
                if not client and candidates:
                    self._progress(
                        f"  ↪ all {len(candidates)} neighbor candidates failed"
                    )
                elif not candidates:
            ...
            if client is None:
                ...
                # Graph: unreachable node + edge from parent
                ...
                if item.parent_hostname:
                    self._graph.add_edge(
                        from_hostname=item.parent_hostname,
                        to_hostname=f"unreachable-{item.ssh_target}",
                        egress_interface=item.parent_interface,
                        next_hop_ip=item.ssh_target,
                        depth=item.depth,
                    )
            ...
            if identity.hostname in self._visited:
                ...
                if identity.hostname in item.ancestors:
                    ...
                    # Graph: convergence edge (node already exists)
                    if item.parent_hostname:
                        self._graph.add_edge(
                            from_hostname=item.parent_hostname,
                            to_hostname=identity.hostname,
            ...
            if hop is not None:
                ...
                # ── Graph: add node and edge from parent ──
                is_connected_g = (hop.is_terminal and hop.route
                                  and hop.route.is_connected)
                egress_list = []
                if hop.resolutions:
                    for res in hop.resolutions:
                        if res.egress_interface:
                            egress_list.append(res.egress_interface.name)
                ...
                self._graph.add_node(
                    ...
                    is_connected=is_connected_g,
                    route_summary=(self._summarize_route(hop.route)
                                   if hop.route else None),
                    fib_summary=(f"programmed, {len(hop.fib.next_hops)} path(s)"
                                 if hop.fib and hop.fib.next_hops else None),
                    egress_interfaces=egress_list,
                )
                if item.parent_hostname:
                    self._graph.add_edge(
                        from_hostname=item.parent_hostname,
            ...
            # Track ECMP (legacy — kept for backward compat, but graph is authoritative)
        ...
        self._graph.completed_at = completed_at

        # Use graph as authoritative ECMP count — it catches convergence-
        # derived branching that the per-hop counter misses
        self._chain.ecmp_branch_points = self._graph.ecmp_branch_count
        ...
        self._emit(HopEvent(
            ...
            ] + ([f"  [#00d4ff]ECMP branch points: "
                  f"{', '.join(self._graph.ecmp_branch_points)}[/]"]
                 if self._graph.ecmp_branch_points else [])
              + ([f"  [#00d4ff]Convergence points: "
                  f"{', '.join(self._graph.convergence_points)}[/]"]
                 if self._graph.convergence_points else [])
              + [f"  [#444444]path {i}: {' → '.join(p)}[/]"
                 for i, p in enumerate(self._graph.enumerate_paths())]
        ...
        if self.config.log_file:
            ...
            # Save forwarding graph alongside diagnostics
            graph_path = self.config.log_file.replace('.json', '.graph.json')
            if graph_path == self.config.log_file:
                graph_path = self.config.log_file + '.graph.json'
            self._graph.save(graph_path)
            logger.info(f"Forwarding graph written to {graph_path}")
    ...
    def _gather_forwarding_state(self, client: SSHClient,
        ...
        # Select next-hop source based on config preference
        fib_nhs = [(nh.address, nh.interface) for nh in fib_entry.next_hops] if fib_entry and fib_entry.next_hops else []
        rib_nhs = [(nh.address, nh.interface) for nh in route_entry.next_hops] if route_entry and route_entry.next_hops else []

        pref = self.config.nh_source_preference
        if pref == "rib":
            if rib_nhs:
                nh_list, nh_source = rib_nhs, "rib"
            elif fib_nhs:
                nh_list, nh_source = fib_nhs, "fib"
            else:
                nh_list, nh_source = [], "none"
        elif pref == "auto":
            # Use whichever has more next-hops — catches ECMP missed by
            # a parser bug on one side (e.g., Junos FIB ulst wrapper)
    ...
    def _resolve_via_neighbors(self, item: QueueItem) -> list[str]:
        """
        ...
        Returns a list of candidate IPs to try, in priority order.
        The caller tests SSH reachability on each until one connects.
        """
        ...
        # Return ALL candidates — caller tests reachability.
        # LLDP management addresses are often in RFC 6598 space
        # (100.64.x.x) which isn't reachable from all networks,
        # so DNS may resolve to a different, reachable IP.
        candidates = []
        mgmt_ip = neighbor.management_address

        if mgmt_ip:
            self._progress(
                f"  ↪ {neighbor.source.upper()}: "
                f"{neighbor.system_name or '?'} → {mgmt_ip}"
            )

        # Always try DNS first — it resolves via /etc/hosts or real DNS
        # and typically returns a routable management IP (10.x.x.x),
        # whereas LLDP management addresses may be RFC 6598 (100.64.x.x).
        ...
        # Add LLDP management address as fallback (after DNS)
        if mgmt_ip and mgmt_ip not in candidates:
    ...
    def _parse_neighbor_detail(output: str, protocol: str) -> Optional[NeighborInfo]:
        ...
        for pattern in (
            ...
            if m:
                system_name = m.group(1).strip().rstrip(',').strip('"\'')
                break
    ...
    def _parse_neighbor_brief(output: str, protocol: str) -> Optional[NeighborInfo]:
        ...
        Junos brief:
          et-0/0/50  ae0  08:05:e2:13:fe:40  INT::... edge1-01.iad1.company.com
        ...
        for candidate in sorted(candidates, key=lambda c: -c.count('.')):
            ...
            if re.match(r"^[a-zA-Z]", candidate):
                system_name = candidate.rstrip(',').strip('"\'')
                break
    ...
    def _resolve_hostname_dns(self, system_name: str) -> Optional[str]:
        ...
        """
        clean_name = system_name.strip().strip('"\'').rstrip('.')
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  fibtrace walk -p 10.0.0.0/24 -s 172.16.1.1 -u admin --password secret\n"
            "  fibtrace walk -p 10.0.0.0/24 -s 172.16.1.1 -u admin --password secret -v\n"
            "  fibtrace walk -p 10.0.0.0/24 -s 172.16.1.1 -u admin --password secret "
            "--log /tmp/ft.json\n"
        ),
    ...
    parser.add_argument("--nh-source", default="fib",
                        choices=["fib", "rib", "auto"],
                        help="Next-hop source preference: fib (default), "
                             "rib, or auto (use whichever has more next-hops)")
    parser.add_argument("--domain", default=None,
                        help="DNS domain suffix for neighbor hostname resolution "
                             "(e.g., 'company.com' resolves 'edge03' as "
                             "'edge03.company.com')")
    ...
    parser.add_argument("--graph", default=None,
                        help="Write forwarding graph JSON to file")
    parser.add_argument("--dot", default=None,
                        help="Write Graphviz DOT to file")
    ...
    config = WalkerConfig(
        ...
        nh_source_preference=args.nh_source,
    ...
    if args.json:
        ...
        output = {
            ...
            "graph": walker.graph.to_dict() if walker.graph else None,
        }
    ...
    # ── Graph output (independent of --json) ──
    if args.graph and walker.graph:
        walker.graph.save(args.graph)
        print(f"Forwarding graph written to {args.graph}", file=sys.stderr)

    if args.dot and walker.graph:
        with open(args.dot, 'w') as f:
            f.write(walker.graph.to_dot())
        print(f"Graphviz DOT written to {args.dot}", file=sys.stderr)
